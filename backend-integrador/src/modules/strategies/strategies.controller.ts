...
export class StrategiesController {
  ...
  @Get() @ApiQuery({ name: 'tipo', required: false, enum: StrategyType }) async list(
    @Query('tipo') tipo?: StrategyType,
  ) {
    const repo = this.dataSource.getRepository(Strategy);
    return await repo.find({ where: tipo ? { tipo } : {} });
  }
