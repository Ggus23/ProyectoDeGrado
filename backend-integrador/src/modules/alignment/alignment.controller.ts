...
export class AlignmentController {
  ...
  @Post() async create(@Body() dto: AlignmentDto) {
    const unit = await this.dataSource
      ...
      .findOne({ where: { id: dto.pgfUnitId } });
    if (!unit) throw new BadRequestException('Unidad no encontrada');
    let rubric: Rubric | null = null;
    if (dto.rubricId)
      rubric = await this.dataSource.getRepository(Rubric).findOneBy({ id: dto.rubricId });
    const repo = this.dataSource.getRepository(Alignment);
    ...
    if (!saved.rubric)
      throw new BadRequestException('Cada evidencia debe enlazar al menos 1 rúbrica/criterio');
    return saved;
  }
  @Get() @ApiQuery({ name: 'pgfUnitId', required: true }) async list(
    ...
    return await this.dataSource
      .getRepository(Alignment)
      .find({ where: { pgfUnit: { id: pgfUnitId } }, relations: ['rubric'] });
  }
}
